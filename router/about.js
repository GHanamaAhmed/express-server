...
aboutRouter.put("/aboutMe", async (req, res) => {
  ...
  if (error) {
    ...
    if (aboutMeData.length === 0) {
      ...
      await ModuleAboutMe.updateOne({}, { $set: { text: aboutMe.text } })
        .then(() => {
...
aboutRouter.post("/workExperience", async (req, res) => {
  const { error } = schemaValidateWorkExperience.validate(
...
aboutRouter.delete("/workExperience", async (req, res) => {
  ...
  if (id == null) {
    ...
    await ModuleWorkExperience.findByIdAndDelete(id)
      .then(() => {
  ...
  if (error) {
    ...
    if (req.body?.id == null) {
      ...
      if (workExperienceData == null) {
        ...
        await ModuleWorkExperience.findByIdAndUpdate(id, workExperience)
          .then(async () => {
...
aboutRouter.delete("/education", async (req, res) => {
  ...
  if (id == null) {
    ...
    await ModuleEducation.findByIdAndDelete(id)
      .then(() => {
        res.json({ id }).status(200);
      })
      .catch((err) => {
...
aboutRouter.put("/education", async (req, res) => {
  ...
  if (req.body?.id == null) {
    ...
    if (educationData == null) {
      ...
      await ModuleEducation.findByIdAndUpdate(id, education)
        .then(() => {
